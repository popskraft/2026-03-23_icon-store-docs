...
| **Локальная обработка фона** | Удаление/очистка фона в Python worker         | Входит в стоимость worker     |
...
| **Инструменты** | n8n, PostgreSQL, Claude API, Amazon SP-API, BigCommerce API v3, S3/R2 |
...
| Удаление фона | Локально в Python worker (Pillow/libvips) + ручной fallback | ✅ Подтверждён 2026-03-24 |
...
**Итого MVP: ~$104–137/month** [DEFAULT — подтвердить потолок с owner]
...
| ✅ | Remove.bg/rembg не используются; очистка фона локально в Python worker, сложные кейсы — ручная ретушь |
